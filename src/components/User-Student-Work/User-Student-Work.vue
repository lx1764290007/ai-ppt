<template>
  ...
  <div class="table-header student-work--header">
    <div class="on--back" v-if="showWorkItem && searchParams.type === WORK_TYPE.STUDENT">
    ...
    <div class="student-work--right_side" v-if="!showWorkItem">
      ...
      <div class="student-work--right_side_student" v-show="searchParams.type === WORK_TYPE.STUDENT">
        <el-select
          ...
          <el-option v-for="(item, i) of gradeList"
                     :key="i"
                     :label="locale === 'en'? $t('grade.grade') + item.gradeName:item.gradeName + $t('grade.grade')"
                     :value="item.id">
            {{ locale === "en" ? $t("grade.grade") + item.gradeName : item.gradeName + $t("grade.grade")}}
          </el-option>
        </el-select>
        <el-select
          clearable
  ...
  <div class="student-work--content" style="height: calc(100vh - 60px - 60px - 20px);"  v-if="showWorkItem && searchParams.type === WORK_TYPE.STUDENT">
    <div class="student-work--item" v-for="item of workList" :key="item.id" @click="onShowAiEvaluate(item)">
      ...
      <div class="student-work--item_bottom">
        ...
        <p style="padding-top: 2px">{{ dayJS(item.createTime).format("YYYY/MM/DD HH:mm:ss") }}</p>
      </div>
    </div>
  ...
  <div class="student-work--content"  v-if="searchParams.type === WORK_TYPE.FREE">
    ...
    <div class="student-work--item" v-for="item of freeWorkList" :key="item.id" @click="onShowAiEvaluate(item)">
      <div class="production-img">
  ...
  <div class="student-work--content"  v-if="searchParams.type === WORK_TYPE.THEME">
    ...
    <p class="empty-text" v-if="showEmpty_themes">empty</p>
  </div>
...
const searchParams = reactive<SearchParams>({
  ...
  userId: props.userId,
  name: "",
  type: WORK_TYPE.STUDENT as 1 | 2 | 3,
  gradeId: undefined,
  classesId: undefined,
  themeId: undefined
});
...
const getGradeList = (isGetClasses: boolean = false) => {
  ...
  fetchGradeList({
    userId: props.userId
  }).then((res: Http.Response<GradeItem[]>) => {
    if (res && res.code === 200) {
      gradeList.value = res.data.map((it: GradeItem)=>{
        return {
          ...it
        }
      });
...
.production-img {
  width: 100%;
  overflow: hidden;
  max-height: calc(216px - 62px);
  min-height: calc(216px - 62px);
}
.free-creation-img {
  max-height: 70%;
  min-height: 70%;
