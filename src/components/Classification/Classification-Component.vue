    ...
    <div class="modal-wrapper">
      <div class="subject-editor__header">
        <span class="subject-editor__header--title">
          {{ !data?.id ? $t("subject._add") : "" }}{{ $t("subject.title")
          }}{{ data?.id ? $t("subject.edit") : $t("subject.add") }}
        </span>
...
const props = defineProps<{
  ...
  currentGrade?: number
  ppt?: boolean
  gradeList: { label: string, id: number,currentId?: number }[]
}>();
...
const getErrTips = () => t("subject.emptyName");
const onSubmit = () => {
  // "name":"李克勤dddd2",
  //   "topic":10,
  //   "question_types":130,
  ...
  if (suject.value.trim() && suject.value.length > 0) {
    ...
    props.data?.id ? fetchTopicUpdate(formData).then((res: Http.Response<any>) => {
      ...
      fetchTopicAdd(formData).then(res => {
        res.code === 200 && emit("close", true);
      }).finally(() => {
        //   modalVisible.value = false;
        loading.value = false;
