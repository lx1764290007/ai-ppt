<template>
  <div class="container" v-show="state===0">
    ...
    <div class="ppt-list__container">
      ...
      <!--      </div>-->
      <div style="height: calc(100vh - 60px - 20px - 56px);position: relative;overflow-y: hidden">
        <el-empty :description="$t('universal.empty')" style="margin: 100px auto" v-if="!loading && dataSource.length<1" />
        <div class="ppt-list__wrapper">

          <div class="tab-pane" @click.self="pptPreviewItem = null" :style="{'justify-content':dataSource.length<4? 'flex-start':'space-between'}">
            <div @click.prevent.stop="()=>openPreview(it)" @dblclick.prevent.stop="toGoogleSlide(it.pptid, it.gptJson)"
                 :class="{'panes-item':true,'panes-item-active':pptPreviewItem?.pptid === it?.pptid}"
              ...
              <div :class="{'tabs-item':true,'tabs-item-without-bs':pptPreviewItem?.pptid === it?.pptid}">
                ...
                <div class="item-info" @click.stop>
                  <div style="display: flex;flex-flow: column;justify-content: flex-start;align-items: flex-start">
                    <span :title="it.pptname"
                          style="text-align:left;width: 180px;display: inline-block;overflow: hidden;white-space: nowrap;text-overflow: ellipsis">{{ it.pptname }}</span>
                    <span
                      style="color:#5e5e5e;font-size: smaller;">{{ dayjs(it.createtime).format("YYYY-MM-DD HH:mm:ss") }}</span>
                  </div>
                  <ContextMenu @select="onContextSelect($event, it)" ppt />
                </div>
              </div>
            </div>
<!--            <div class="panes-item" @click.stop="onAddPPT" title="Ai PPT"-->
<!--                 :style="{pointerEvent:removeAble? 'none':'auto'}">-->
<!--              <div class="tabs-item">-->
...
<!--            </div>-->
          ...
          <div :class="{'ppt-preview':true, 'ppt-preview-show':pptPreviewItem}">
            <div class="ppt-preview-content"
                 v-html="getPreview(pptPreviewItem?.pptOutline ||pptPreviewItem?.gptJson ,pptPreviewItem?.pptname)"
                 v-show="pptPreviewItem">
            </div>
            <div class="preview-button" v-show="pptPreviewItem">
              <el-button type="primary" class="default__button-primary" style="width: 100px"
                         @click.stop="toGoogleSlide(pptPreviewItem?.pptid, pptPreviewItem?.gptJson)">
                {{ $t("ppt.enter") }}
              </el-button>
  ...
  <PPTEditor :subjectId="folder?.themeId" :user-id="userInfo.getUserInfo?.user?.id" v-if="state!==0" :grade="grade" :grades="gradeList" @back="onChangeStateTo0"
             :ppt-name="activeItem.pptName" :ppt-id="activeItem.id" />
  ...
  <popup-component v-model:modal-value="showConfirm">
    <div class="confirm-wrapper">
      ...
      <el-space style="display: flex;margin-top: 30px;margin-left: 8px">
        <el-button class="buttons-primary" type="primary" :loading="deleteLoading" style="width: 90px"
                   @click="()=>removePPT()">
          {{ $t("home.confirm") }}
        </el-button>
...
import { fetchAddPPT, fetchPPTAndThemeList, fetchRemovePPT } from "@/http/ppt";
...
const toGoogleSlide = (id: number, json: string) => {
  ...
  try {
    ...
    const newJson = _json.map((item: { title: string, content: any[] }) => {
      return {
        imageDescription: item.content?.find((it: any) => Boolean(it.imageKeyWord && it.imageKey))?.imageKeyWord || ""
      };
    });
