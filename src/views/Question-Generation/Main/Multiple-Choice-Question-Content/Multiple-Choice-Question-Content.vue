<template>
  <div :class="{ 'multiple-choice-question-wrapper': true, topic__preview: preview }">
    <div class="multiple-choice-question-content">
      ...
      <div class="multiple-choice-question-content-img">
        ...
        >
          ...
          <div class="el-upload__text" style="color: #fff">{{$t('questionEdit.drag')}} {{$t('questionEdit.or')}}<em> {{$t('questionEdit.click')}} </em>{{$t('questionEdit.picture')}}</div>
        </el-upload>
        <div class="multiple-choice-question-content-uploaded" v-else>
          ...
          <div class="multiple-choice-question-content-tools" v-if="!preview">
            <img src="@/assets/crop.png" alt="crop" crossorigin="anonymous" class="tools-btn"  @click.stop="onShowCropper" />
            <img src="@/assets/info.png" alt="crop" crossorigin="anonymous" class="tools-btn"  @click.stop="onShowInfo" />
            <img src="@/assets/remove.png" alt="crop" crossorigin="anonymous" class="tools-btn"  @click.stop="onDeletePicture" />
          </div>
        </div>
      </div>
    </div>
    <div class="multiple-choice-question-content-footer">
      ...
      <div class="multiple-choice-question-item-button" @click.stop="onPowerClick" v-if="!preview && dataSource.options && dataSource.options.length !== 4">
        {{ dataSource.options && dataSource.options.length > 4 ? $t("questionEdit.remove") :  $t("questionEdit.add") }}{{$t('questionEdit.optionalAnswer')}}
      </div>
    </div>
  </div>
  ...
  <copyright-popup v-model="copyrightVisible" @done="onCopyrightDone" :data="{origin:dataSource.displayLink||dataSource.images?.[0]?.title, copyright: dataSource.copyRight}"></copyright-popup>
</template>
...
const onAiPictureChange = (url: {image:string, title:string}) => {
  topicStore.update(Object.assign({}, props.dataSource, { images: [{image:url.image}] , displayLink: url.title}), topicStore.getActive());
