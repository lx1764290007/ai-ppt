  ...
  <div class="subject-page-wrapper">
    <div class="row">
      <div>
        ...
        <span class="title">
          {{ dataProp?.subjectName || locale === "en" ? dataProp?.subjectNameEn : locale === "zh_hant" ? dataProp?.subjectNameHk : dataProp?.subjectNameCn }}
        </span>
        <el-input
        ...
        <main v-if="!topicTypeLoading && topicTypeDataSource.length > 0" class="subject-item-wrapper"
          ...
          >
            <div class="content-menu" @click.stop>
              <ContextMenu color="#444" vertical @select="onContextSelect($event, item)" world :file-id="item.docId">
              </ContextMenu>
            </div>
            <div :class="{'subject-item-img':true}" :title="item.typeName">
              <el-image
                :scroll-container="scrollRef"
                :src="item.typeImg"
                @click.stop="toGoogleSlide(item.docId)"
                class="preview--image"
...
onBeforeMount(() => {
  backTitle = props.dataProp?.subjectName || locale.value === 'en'? props.dataProp?.subjectNameEn : locale.value === 'zh'? props.dataProp?.subjectNameCn:props.dataProp?.subjectNameHk;
...
const sectionHeight = computed(() => {
  ...
  return `calc(100vh - 75px - ${headerHeight}px - 60px)`;
});
...
const toGoogleSlide = (id: number) => {
  const base_edit_url_start = "https://docs.google.com/document/d/";
  const base_edit_url_end = "/edit";
  const url = `${base_edit_url_start}${id}${base_edit_url_end}`;
